...
func NewPostgresDB() (*PostgresDB, error) {
	...
	if dsn == "" {
		...
		// settlements/signing_requests/etc. for `app`, scoped by a
		// per-transaction session variable this service doesn't set. It
		// processes settlements for whichever customer's signed
		// transaction it's broadcasting, not one fixed authenticated
		// tenant, so app_admin (BYPASSRLS) preserves today's real
		// behavior until it's threaded through set_config() the way
