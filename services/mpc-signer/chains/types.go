...
type Signature struct {
	...
	V              byte   `json:"v"`               // recovery ID (Ethereum) or variant
...
type ChainSignRequest struct {
	...
	Message  []byte                 `json:"message"`   // pre-built message bytes (RLP, script, instruction, SignDoc)
...
type ChainSignResponse struct {
	...
	SignedTx   string `json:"signed_tx,omitempty"`  // full signed transaction (hex)
...
// NewChainSigner returns the appropriate ChainSigner for a given chain.
