...
func NewPostgresDB() (*PostgresDB, error) {
	...
	if dsn == "" {
		...
		// integrations/integration_webhook_logs for `app`, scoped by a
		// per-transaction session variable this service doesn't set even
		// though every call already receives a customerID. app_admin
		// (BYPASSRLS) preserves today's real behavior until that
		// customerID is threaded through set_config() the way
