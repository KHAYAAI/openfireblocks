...
| The supporting services answer honestly | `infrastructure/kind/services-smoke-test.sh` drives 19 endpoints across all six from inside the cluster. It found three endpoints returning 500 for a request naming something that does not exist, and one returning the driver's `pq: invalid input syntax ... (22P02)` in the response body; all now classify 400/404/500 with detail kept in the logs |
